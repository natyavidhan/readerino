...
#include "Sd2Card.h"
//------------------------------------------------------------------------------
// readerino: SDCARD_SPI/settings must exist regardless of SOFTWARE_SPI --
// chipSelectHigh/Low, init(), setSckRate() etc. further down reference them
// unconditionally (guarded by USE_SPI_LIB, not SOFTWARE_SPI, in this library
// version). They only do harmless transaction bookkeeping around the
// bit-banged transfers in software mode -- no data crosses the hardware
// peripheral's physical pins unless spiSend/spiRec below actually call
// .transfer(), which they don't when SOFTWARE_SPI is defined.
#ifdef USE_SPI_LIB
  #ifndef SDCARD_SPI
