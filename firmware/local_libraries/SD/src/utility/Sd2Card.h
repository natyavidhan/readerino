...
// readerino: force the library's own bit-banged SPI path unconditionally,
// on every translation unit that includes this header -- including this
// library's own .cpp files, which are compiled independently of anything
// our sketch code defines before its own #include <SD.h>. See the pin
// remap further down for why (frees the hardware SPI bus for the TFT).
#ifndef SOFTWARE_SPI
#define SOFTWARE_SPI
#endif
#include "Sd2PinMap.h"
...
#else  // SOFTWARE_SPI
  // readerino: repointed off the default 11/12/13 so the hardware SPI bus
  // is free for the TFT (which needs the throughput far more than the SD
  // card does -- see App/Display's redraw path). CS stays on 10, matching
  // the wiring that was already in place.
  /** SPI chip select pin */
  uint8_t const SD_CHIP_SELECT_PIN = 10;
  /** SPI Master Out Slave In pin */
  uint8_t const SPI_MOSI_PIN = 19; // A5
  /** SPI Master In Slave Out pin */
  uint8_t const SPI_MISO_PIN = 14; // A0
  /** SPI Clock pin */
  uint8_t const SPI_SCK_PIN = 18; // A4
#endif  // SOFTWARE_SPI
//------------------------------------------------------------------------------
